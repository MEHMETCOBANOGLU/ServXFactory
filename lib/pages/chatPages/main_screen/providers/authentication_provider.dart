...
class AuthenticationProvider extends ChangeNotifier {
  ...
  Future<void> saveUserDataToSharedPreferences() async {
    ...
    await sharedPreferences.setString(
        'userModel', jsonEncode(userModel!.toMap()));
  }
    ...
    try {
      if (fileImage != null) {
        ...
        String imageUrl = await storeFileToStorage(
            file: fileImage,
            // reference: '${Constants.userImages}/${userModel.id}');
            reference: '${userModel.image}/${userModel.id}');
  ...
  Future<void> sendFriendRequest({
    ...
    try {
      ...
      await _firestore.collection('users').doc(friendID).update({
        'friendRequestsUIDs': FieldValue.arrayUnion([_uid]),
      });

      // add friend uid to our friend requests sent list
      await _firestore.collection('users').doc(_uid).update({
        'sentFriendRequestsUIDs': FieldValue.arrayUnion([friendID]),
      });
    } on FirebaseException catch (e) {
      print(e);
    }
  ...
  Future<void> cancleFriendRequest({required String friendID}) async {
    try {
      ...
      await _firestore.collection('users').doc(friendID).update({
        'friendRequestsUIDs': FieldValue.arrayRemove([_uid]),
      });

      // remove friend uid from our friend requests sent list
      await _firestore.collection('users').doc(_uid).update({
        'sentFriendRequestsUIDs': FieldValue.arrayRemove([friendID]),
      });
    } on FirebaseException catch (e) {
      print(e);
    }
  ...
  Future<void> acceptFriendRequest({required String friendID}) async {
    ...
    await _firestore.collection('users').doc(friendID).update({
      'friendsUIDs': FieldValue.arrayUnion([_uid]),
    });

    // add friend uid to our friends list
    await _firestore.collection('users').doc(_uid).update({
      'friendsUIDs': FieldValue.arrayUnion([friendID]),
    });

    // remove our uid from friends request list
    await _firestore.collection('users').doc(friendID).update({
      'sentFriendRequestsUIDs': FieldValue.arrayRemove([_uid]),
    });

    // remove friend uid from our friend requests sent list
    await _firestore.collection('users').doc(_uid).update({
      'friendRequestsUIDs': FieldValue.arrayRemove([friendID]),
    });
  }
  ...
  Future<void> removeFriend({required String friendID}) async {
    ...
    await _firestore.collection('users').doc(friendID).update({
      'friendsUIDs': FieldValue.arrayRemove([_uid]),
    });

    // remove friend uid from our friends list
    await _firestore.collection('users').doc(_uid).update({
      'friendsUIDs': FieldValue.arrayRemove([friendID]),
    });
  }
  ...
  Future<List<UserModel>> getFriendsList(
    ...
    List<dynamic> friendsUIDs = documentSnapshot.get('friendsUIDs');
  ...
  Future<List<UserModel>> getFriendRequestsList({
    ...
    List<dynamic> friendRequestsUIDs =
        documentSnapshot.get('friendRequestsUIDs');
