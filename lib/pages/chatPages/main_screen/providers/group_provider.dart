...
import 'package:ServXFactory/pages/chatPages/main_screen/models/group_model.dart';
...
class GroupProvider extends ChangeNotifier {
  ...
  void addMemberToGroup({required UserModel groupMember}) {
    ...
    _groupModel.membersUIDs.add(groupMember.id);
    notifyListeners();
  ...
  void addMemberToAdmins({required UserModel groupAdmin}) {
    ...
    _groupModel.adminsUIDs.add(groupAdmin.id);
    notifyListeners();
  ...
  Future<void> removeGroupMember({required UserModel groupMember}) async {
    ...
    _groupModel.membersUIDs.remove(groupMember.id);
    notifyListeners();
  ...
  void removeGroupAdmin({required UserModel groupAdmin}) {
    ...
    _groupModel.adminsUIDs.remove(groupAdmin.id);
    notifyListeners();
  ...
  List<String> getGroupMembersUIDs() {
    return _groupMembersList.map((e) => e.id).toList();
  }

  // get a list UIDs from group admins list
  List<String> getGroupAdminsUIDs() {
    return _groupAdminsList.map((e) => e.id).toList();
  }
  ...
  Future<void> acceptRequestToJoinGroup({
    ...
    await _firestore.collection('groups').doc(groupId).update({
      'membersUIDs': FieldValue.arrayUnion([friendID]),
      'awaitingApprovalUIDs': FieldValue.arrayRemove([friendID])
    });
  ...
  Future<void> exitGroup({
    ...
    await _firestore.collection('groups').doc(_groupModel.groupId).update({
